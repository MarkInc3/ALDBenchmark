...
const LOW = ethers.parseUnits("100",18); // 5k
...
describe('# IL LOSS CHECK', () => {
    ...
    it('LOW POOL', async () => {
        ...
        const initPoolVaule = (Number(initBtcAmount) * Number(btcPrice)) +  Number(initUsdtAmount);
        ...
        for(let i=1 ; i <= 1 ;i++){ 
            ...
            console.log('RESULT IL LOSS %: ', -(IR1 / IR2),'\n');
            data[i] = {
                price : Number(afterEstimatedPrice),
                loss :  Number(((IR1 / IR2)))
            };
            console.log(Number(initBtcAmount),'||||', Number(initUsdtAmount))
        }
        OUT.push({
            id : 1,
            data : data
    ...
    //     // console.log('BTC RATE -> ', ethers.formatUnits(estimated[0],18),'  USDT RATE -> ',ethers.formatUnits(estimated[1],6))
    //     let initBtcAmount = ethers.formatUnits(MID,18);
    //     let initUsdtAmount = ethers.formatUnits(SMID,6);
    //     let btcPrice =  ethers.formatUnits(estimated[1],6)
    //     const initPoolVaule = (Number(initBtcAmount) * Number(btcPrice)) +  Number(initUsdtAmount);
    //     console.log('#1 PRE POOL [INIT] VALUE : ',  initPoolVaule);
    ...
    //         // 원래 유동성 유지시의 가치
    //         initBtcAmount = ethers.formatUnits(MID,18);
    //         initUsdtAmount = ethers.formatUnits(SMID,6);
    //         const IR2 = ((Number(initBtcAmount) * Number(afterEstimatedPrice)) +  Number(initUsdtAmount) );
    //         console.log('RESULT IL LOSS %: ', -(IR1 / IR2),'\n');
    //         data[i] = {
    //             price : Number(afterEstimatedPrice),
    ...
    //     // console.log('BTC RATE -> ', ethers.formatUnits(estimated[0],18),'  USDT RATE -> ',ethers.formatUnits(estimated[1],6))
    //     let initBtcAmount = ethers.formatUnits(TOP,18);
    //     let initUsdtAmount = ethers.formatUnits(STOP,6);
    //     let btcPrice =  ethers.formatUnits(estimated[1],6)
    //     const initPoolVaule = (Number(initBtcAmount) * Number(btcPrice)) +  Number(initUsdtAmount);
    //     console.log('#1 PRE POOL [INIT] VALUE : ',  initPoolVaule);
    ...
    //         // 원래 유동성 유지시의 가치
    //         initBtcAmount = ethers.formatUnits(TOP,18);
    //         initUsdtAmount = ethers.formatUnits(STOP,6);
    //         const IR2 = ((Number(initBtcAmount) * Number(afterEstimatedPrice)) +  Number(initUsdtAmount) );
    //         console.log('RESULT IL LOSS %: ', -(IR1 / IR2),'\n');
    //         data[i] = {
    //             price : Number(afterEstimatedPrice),
